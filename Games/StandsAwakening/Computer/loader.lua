...
local function NoCooldownName(Name)
...
local MobBox = Tabs.Stands:AddRightGroupbox('Mod (In Analized)')
MobBox:AddToggle('AB', {
    Text = 'Punch No CD',
    Default = false,
    Tooltip = 'Turn on to be locked',
    Callback = function(state)
        local NoCooldownHP = state
        if NoCooldownHP then
            if getexploit() == 'Krnl UWP' or 'Valyse' or 'Electron' then
                Libray:Notify('Decompress verification: true')
                while wait(.5) and NoCooldownHP do
                    NoCooldownName('PunchDebounce')
                end
            else
                Libray:Notify('Decompress verification: false')
            end
        end
    end
})
    ...
    Callback = function(state)
        ...
        if NoCooldownHP then
            if getexploit() == 'Krnl UWP' or 'Valyse' or 'Electron' then
                Libray:Notify('Decompress verification: true')
                while wait(.5) and NoCooldownHP do
                    NoCooldownName('BarrageDebounce')
                end
            else
                Libray:Notify('Decompress verification: false')
            end
    ...
    Callback = function(state)
        ...
        if NoCooldownHP then
            if getexploit() == 'Krnl UWP' or 'Valyse' or 'Electron' then
                Libray:Notify('Decompress verification: true')
                while wait(.5) and NoCooldownHP do
                    NoCooldownName('HeavyPunchDebounce')
                end
            else
                Libray:Notify('Decompress verification: false')
            end
...
for _, v in pairs(game:GetService("Players"):GetChildren()) do
    if v.Name ~= game.Players.LocalPlayer.Name then
        table.insert(PlayerFarm, v.Name)
    end
end
KillPlayerBox:AddDropdown('PLRNAME', {
    ...
    Callback = function(Value)
        getgenv().PlayerSelectedToKill = Value
    end
})
local Button = KillPlayerBox:AddButton({
    Text = 'Refresh DropDown',
    Func = function()
        Options.PLRNAME:SetValues(PlayerFarm)
    end,
    DoubleClick = false,
...
local Button = KillPlayerBox:AddButton({
    ...
    Func = function()
        ...
        Character:PivotTo(game.Players[getgenv().PlayerSelectedToKill].Character:PivotTo())
        wait(1)
